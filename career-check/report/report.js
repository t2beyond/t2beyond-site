...
function renderReport(report){
  ...
  const actionsHtml=actionPairs.map((a)=>`<li><div><strong>${a.title}</strong><p>${a.action}</p><span>À tester dans les 30 prochains jours.</span></div></li>`).join('');

  const summaryText=`${pLib.base} ${profileGoalText(profile,gKey,dimensions[priorityKeys[0]].name)}`;
  ...
  const html=`
    ...
    <section class="report-section report-page-break"><p class="section-kicker">PAGE 2 · PROFIL</p><h2>Ton profil T2BEYOND en un coup d’œil</h2><p class="section-intro">Une lecture globale de tes forces et des dimensions à renforcer pour atteindre ton objectif.</p><div id="radarHost" class="report-radar"></div><div class="report-dimensions">${dimsHtml}</div><div class="insight-box"><span>CE QUE MONTRE TON PROFIL</span><p>Ton profil s’appuie particulièrement sur <strong>${s1}</strong> et <strong>${s2}</strong>. Les écarts les plus utiles à travailler aujourd’hui se situent sur <strong>${p1}</strong> et <strong>${p2}</strong>.</p><p>Ton enjeu n’est pas de tout améliorer : il est de <strong>concentrer ton effort sur les dimensions les plus décisives pour ton objectif</strong>.</p></div></section>
